...
生成文件为 `desktop/publish/win-x64/DuxiuLedger.exe`，压缩包为 `desktop/publish/DuxiuLedger-v0.4.0-win-x64.zip`。目标电脑不需要安装 .NET Runtime 或 Windows App SDK Runtime。
...
安装程序输出到 `desktop/publish/installer/DuxiuLedger-Setup-v0.4.0-win-x64.exe`，支持当前用户安装、开始菜单快捷方式、可选桌面快捷方式和卸载。

当前版本为 `v0.4.0`，版本变更记录位于仓库根目录的 `CHANGELOG.md`。
