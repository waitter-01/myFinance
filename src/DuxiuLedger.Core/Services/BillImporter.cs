...
public sealed class BillImporter
{
    ...
    private static ImportPreviewResult ParseRows(List<List<string>> rows, string source)
    {
        ...
        int date = Find(headers, "时间", "日期", "交易时间"), amount = Find(headers, "金额", "收支金额", "金额(元)"), type = Find(headers, "收支", "交易类型", "类型"), merchant = Find(headers, "交易对方", "商户", "商品", "备注"), note = Find(headers, "备注", "商品说明", "描述"), category = Find(headers, "分类", "交易分类"), months = Find(headers, "订阅月数", "计费月数", "覆盖月数"), recurringType = Find(headers, "周期支出类型", "周期类型"), coverageStart = Find(headers, "覆盖开始", "费用开始"), nextPayment = Find(headers, "下次付款", "下次支付"), essential = Find(headers, "必要支出", "是否必要");
        ...
        for (var index = headerIndex + 1; index < rows.Count; index++)
        {
            ...
            var kind = ValueAt(values, type); var direction = MapTransactionType(kind); var merchantText = ValueAt(values, merchant); var noteText = ValueAt(values, note); var categoryValue = ValueAt(values, category); var categoryText = string.IsNullOrWhiteSpace(categoryValue) ? "未分类" : categoryValue; var subscriptionMonths = int.TryParse(ValueAt(values, months), out var parsedMonths) ? Math.Max(1, parsedMonths) : 1;
            var record = new TransactionRecord { OccurredOn = when, Direction = direction, Amount = Math.Abs(money), Category = categoryText, Merchant = merchantText, Note = noteText, Source = source, SubscriptionMonths = subscriptionMonths, RecurringType = ValueAt(values, recurringType), CoverageStart = TryDate(ValueAt(values, coverageStart), out var parsedStart) ? parsedStart.Date : null, NextPaymentDate = TryDate(ValueAt(values, nextPayment), out var parsedNext) ? parsedNext.Date : null, IsEssential = Contains(ValueAt(values, essential), "是", "必要", "true", "1") }; record.Fingerprint = TransactionFingerprint.Create(record); result.Add(record);
        }
    ...
    private static string ValueAt(IReadOnlyList<string> values, int index) => index >= 0 && values.Count > index ? values[index].Trim() : "";
