<div align="center">
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/版本-v0.12.0-6B7280" alt="v0.12.0">
  </p>
</div>

## 项目简介

独秀账本是一款面向个人用户的 Windows 桌面记账应用。账本默认保存在本机，无需服务器和在线账户；需要多端使用时，可由用户自行配置 S3 或 S3 兼容对象存储。应用支持导入 Excel、CSV，以及识别微信、支付宝和部分银行的账单截图。
...
- 截图识别：本地识别微信、支付宝、中信银行和工商银行账单截图，支持选择文件、拖拽、剪贴板粘贴、自动分段、预览修正和重复过滤。
- 格式识别：识别微信、支付宝及两类银行页面的常见账单表头以及项目标准模板。
...
当前版本为 **v0.12.0**，新增中信/工商银行截图识别、银行卡与支付平台同笔判断，以及支持多设备并发保护的自动 S3 合并。完整变更内容参见 [CHANGELOG.md](CHANGELOG.md)。
...
| 支付平台/银行截图 | 基础可用 | 微信、支付宝、中信银行、工商银行，本地 OCR、超长图分段、预览校正和去重 |
| 导入去重 | 可用 | 重复导入不会重复保存 |
| 流水搜索 | 可用 | 支持交易对方和备注 |
...
| S3 对象同步 | 可用 | 按记录双向合并、并发写入重试、删除同步、修改后自动同步和定时拉取 |
...
artifacts\DuxiuLedger-v0.12.0-win-x64.zip
```
...
```text
artifacts\installer\DuxiuLedger-Setup-v0.12.0-win-x64.exe
```
...
```powershell
git tag -a v0.12.0 -m "版本：发布 v0.12.0"
git push origin master
git push origin v0.12.0
```

6. 在 GitHub Releases 中使用相同标签创建发行版，仅上传 ZIP 和安装程序；体积较大的单文件 EXE 只保留在本地构建目录。发布说明以 `CHANGELOG.md` 对应版本内容为准。
...
选填字段包括交易对方、商品说明、分类、周期支出类型、覆盖月数、覆盖开始、下次付款、必要支出和备注。例如季度房租的覆盖月数填写 `3`，年度会员填写 `12`；微信、支付宝官方导出的账单可直接尝试导入，不必先转换为标准模板。
...
在“偏好设置 → S3 对象存储同步”中开启同步并填写配置，然后依次点击“保存并测试连接”和“立即双向同步”。应用会把一个带版本号的 JSON 同步对象保存到用户指定的 Bucket 中，不需要创建数据表。开启“自动合并本地修改”后，每次保存会在约 10 秒后合并上传，并每 5 分钟检查其他设备的修改。
...
Secret Key 和 Session Token 只通过 Windows DPAPI 加密保存在当前 Windows 用户配置中，不会写入仓库、日志或同步对象。更换电脑时需要重新输入凭据。应用采用本地优先模式，断网时仍可记账，恢复网络后再合并。不同记录取并集；同一记录在多设备被修改时保留更新时间较新的完整版本；云端对象被另一设备抢先写入时会重新读取并重试，不会直接用旧快照覆盖。
